/**
 * OpenAI Compatible 基础用法 — Vercel AI SDK (@ai-sdk/openai-compatible)
 *
 * 本示例演示如何通过七牛 AIToken 平台，使用 Vercel AI SDK 的 OpenAI Compatible Provider
 * 调用 OpenAI 兼容模型进行文本生成。
 *
 * @ai-sdk/openai-compatible 是一个通用的 OpenAI 兼容 Provider，专为第三方 OpenAI 兼容 API 设计。
 * 与 @ai-sdk/openai 相比，它不含 OpenAI 专有逻辑（如 Responses API 自动检测、
 * "o" 系列模型的 developer 角色转换），因此更适合七牛 AIToken 等非 OpenAI 平台。
 *
 * 功能覆盖：
 * - 非流式生成：一次性返回完整响应
 * - 流式生成：逐步返回响应内容，降低首字延迟
 * - 系统提示词：自定义模型角色和行为（无需 providerOptions 变通）
...
// 创建 OpenAI Compatible Provider 实例，指向七牛 AIToken 平台
// name 参数用于标识 provider，会出现在错误信息和日志中
...
// 获取模型实例
// 与 @ai-sdk/openai 不同，无需调用 .chat() 来避免 Responses API，
// openai-compatible provider 始终使用 Chat Completions API
...
// 与 @ai-sdk/openai 不同，openai-compatible provider 不会根据模型 ID
// 自动将 system 角色转为 developer 角色，因此无需设置
// providerOptions: { openai: { systemMessageMode: "system" } }
// 这是使用 openai-compatible 的一大优势：系统提示词直接生效，无需变通处理
