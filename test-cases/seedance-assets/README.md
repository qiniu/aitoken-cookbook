...
export ACCESS_KEY="your-access-key"                    # AK（火山签名格式的 Access Key）
export SECRET_KEY="your-secret-key"                    # SK（火山签名格式的 Secret Key）
export PROJECT_NAME="your-project"                     # 选填，项目名（需与 AK/SK 有权限的项目一致）
python run_tests.py
```
...
| `ACCESS_KEY` | 无（必填） | AK（火山签名格式的 Access Key） |
| `SECRET_KEY` | 无（必填） | SK（火山签名格式的 Secret Key） |
| `PROJECT_NAME` | `cases.yaml` 的 `project_name`（默认 `default`） | 项目名；需与 AK/SK 有权限的项目一致，否则 403 AccessDenied |
